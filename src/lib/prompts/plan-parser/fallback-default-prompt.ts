...
Some plans include per-week commentary after the main table, marked with → symbols (e.g. →4, →5, →6). IGNORE these sections entirely. Extract sessions ONLY from the main training schedule table, not from explanatory prose.
...
RANGE RULE — applies to ALL range values (distance, duration, pace):
Use the UPPER bound as the primary numeric value.

Distance ranges → upper bound:
  "4-5 miles"  → distance_miles=5, distance_km=8.05
  "8-10 km"    → distance_km=10, distance_miles=6.21

Duration ranges → upper bound for duration_minutes; also store both bounds:
  "60-75 min"  → duration_minutes=75, duration_min_minutes=60, duration_max_minutes=75
  "40-50 min"  → duration_minutes=50, duration_min_minutes=40, duration_max_minutes=50

Pace/intensity ranges → upper bound (larger number = slower pace):
  "8:00-8:30/mile" → intensity="8:30/mile"
  "4:45-5:00/km"   → intensity="5:00/km"
