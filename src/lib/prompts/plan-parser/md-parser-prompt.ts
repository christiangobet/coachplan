...
5. Use the Notes column for execution detail, coaching notes, and priority signals (⭐ = KEY priority).
...
- For each session, copy useful Notes-column text verbatim into notes. This includes execution details, race strategy, logistics, hydration/fueling guidance, and any note that should still be visible on the saved day/activity.
- For each session, if the Notes column contains perceived-effort guidance, technique cues, pacing philosophy, or mental focus instructions, also copy that coaching-focused text verbatim into coaching_note. Leave null if no coaching cue is present.
- When the Notes column includes both execution detail and coaching cues, notes should preserve the full useful note text, while coaching_note may contain the coaching-focused subset.
